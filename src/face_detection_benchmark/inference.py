...
def iter_batches(
    records: list[FrameMetadata] | list[CocoImageRecord],
    batch_size: int,
) -> Iterable[list[FrameMetadata] | list[CocoImageRecord]]:
...
def load_coco_image_batch(
    ...
    for record in records:
        ...
        if image_bgr is None:
            raise ValueError(f"Could not read benchmark image: {record.image_path}")
        images_bgr.append(image_bgr)
        images_rgb.append(cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB))
        loaded_records.append(record)
